...
- Within the script editor or a chunk
  ...
  - `Shift` + `Cmd/Ctrl` + `M` : Inserts a pipe operator.

  - `Cmd/Ctrl` + `Enter` : Run current line or section.

  - `F1` (Note: on a Mac you need to press `fn` + `F1`) : Show help for function at cursor.

  - `Cmd/Ctrl` + `Shift` + `C` : Toggle comment on current line.

- Within R Markdown

  - `Cmd/Ctrl` + `Alt` + `I` : Insert chunk.
...
There is a small red square you can hit in the top right of the chunk, or you can use the Escape key.

### **q4** How do you show the "Document Outline" in RStudio?

There is a document outline button in the top right of the source document.
